...
use Abills::Base qw(check_time parse_arguments gen_time next_month
  days_in_month int2ml in_array show_hash date_diff);
use Abills::Defs;
use Customers;
use Users;
use Admins;
...
sub create_service_orders{
  ...
  my $cross_modules_return = cross_modules_call( '_docs', { %{$docs_user},
      UID          => $docs_user->{uid},
...
sub prepaid_invoices{
  ...
  #Dv
  my @MODULES = ('Dv');
  my Dv $mod_name = $MODULES[0];
  require $mod_name . '.pm';
  $mod_name->import();
  my Dv $Module_name = $mod_name->new( $db, $admin, \%conf );
  ...
  foreach my $line ( @{$list} ){
    ...
    #add  tp invoice
    if ( $TP_LIST->{$tp_id} ){
      my (undef, $fees_sum) = split( /;/, $TP_LIST->{$tp_id} );
      print "  TP_ID: $tp_id FEES: $fees_sum\n" if ($debug > 2);
      push @orders, {
        ORDER => $lang{TARIF_PLAN},
        SUM   => $fees_sum
      };
    }
...
sub get_tps{
  ...
  foreach my $line ( @{$tp_list} ){
    if($attr->{INTERNET}) {
      if($attr->{AGE}) {
        if($line->{age}) {
          $TP_LIST{ $line->{tp_id} } = "$line->{name};$line->{age};". ($line->{month_fee} + $line->{activate_price});
        }
      }
      elsif ($line->{month_fee} > 0) {
        $TP_LIST{ $line->{tp_id} } = "$line->{name};$line->{month_fee}";
      }
      elsif ($line->{day_fee} > 0) {
        $TP_LIST{ $line->{tp_id} } = "$line->{name};" . ($line->{day_fee} * 30);
      }

    }
    else {
      if($attr->{AGE}) {
        $TP_LIST{ $line->{id} } = "$line->{name};$line->{age};". ($line->{month_fee} + $line->{activate_price});
      }
      elsif ($line->{month_fee} > 0) {
        $TP_LIST{ $line->{id} } = "$line->{name};$line->{month_fee}",;
      }
      elsif ($line->{day_fee} > 0) {
        $TP_LIST{ $line->{id} } = "$line->{name};" . ($line->{day_fee} * 30);
      }
    }
  }
...
sub prepaid_invoices_company{
  ...
  foreach my $line ( @{$list} ){
    ...
    my $position_num = 0;
    my @invoice_orders = ();
    foreach my $dv_info ( @{$dv_list} ){
      my $uid   = $dv_info->{uid};
      my $tp_id = $dv_info->{tp_id} || 0;
      my $fio   = $dv_info->{fio} || '';
      $user->{ACTIVATE} = $dv_info->{activate};
      print "UID: $uid LOGIN: $dv_info->{login} FIO: $fio TP: $tp_id\n" if ($debug > 2);
      #Add debetor accouns
      if ( $TP_LIST->{$tp_id} ){
        my ($tp_name, $fees_sum) = split( /;/, $TP_LIST->{$tp_id} );
        $tp_sum += $fees_sum;
        print "  DEPOSIT: $dv_info->{deposit} ABON: $TP_LIST->{$tp_id}\n" if ($debug > 2);
    ...
    if ( $tp_sum > 0 ){
      print "TP SUM: $tp_sum\n" if ($debug > 0);
      $FORM{SUM}   = $tp_sum;
      $FORM{ORDER} = "$lang{DV}: $lang{TARIF_PLAN} ($tp_counts) "
        . get_period_dates({
            TYPE       => 1,
  ...
  foreach my $line ( @{$list} ){
    ...
    push @MULTI_ARR, {
        ...
        ORDER_TOTAL_SUM_VAT => ($conf{DOCS_VAT_INCLUDE})                                            ? sprintf( "%.2f",
            abs( $line->{deposit} / ((100 + $conf{DOCS_VAT_INCLUDE}) / $conf{DOCS_VAT_INCLUDE}) ) ) : 0.00,
...
sub periodic_acts_pre_period {
  ...
  foreach my $line (@$dv_list) {
    $users_tps{$line->{uid}}=$line->{tp_id};
  }
  ...
  my $list = $Internet->list({
    ...
    DV_EXPIRE      => '_SHOW',
    TP_ID          => '_SHOW',
    #GROUP_BY       => in_array('Internet', \@MODULES) ? 'internet.id' : undef,
    PAGE_ROWS      => 1000000,
    REGISTRATION   => '_SHOW',
    COLS_NAME      => 1,
    %LIST_PARAMS
  ...
  foreach my $line ( @$list ) {
    my $expire = $line->{internet_expire} || $line->{dv_expire};
