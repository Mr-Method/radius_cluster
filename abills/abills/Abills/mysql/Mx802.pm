...
sub auth {
  ...
  if ($RAD_REQUEST->{'ERX-Dhcp-Options'}) {
    ...
    if ($CONF->{MX80_IPOE_SWITCH_PORT}) {
      ...
      $user_auth_params->{NAS_PORT_AUTH} = $CONF->{MX80_IPOE_SWITCH_PORT};
  ...
  if (length($self->{FILTER}) > 0) {
    $self->Auth2::neg_deposit_filter_former($RAD_REQUEST, $NAS, $self->{NEG_DEPOSIT_FILTER_ID},
